...
# Load raster targets with error handling to identify which one causes the metadata error
tryCatch({
  tar_load(years_since_fire.tif)
  message("✓ years_since_fire.tif loaded")
}, error = function(e) {
  message("✗ Error loading years_since_fire.tif: ", e$message)
  stop(e)
})
...
tryCatch({
  ...
  message("✓ most_recent_ndvi.tif loaded")
}, error = function(e) {
  message("✗ Error loading most_recent_ndvi.tif: ", e$message)
  stop(e)
})
...
tryCatch({
  ...
  message("✓ monthly_mean_ndvi.tif loaded")
}, error = function(e) {
  message("✗ Error loading monthly_mean_ndvi.tif: ", e$message)
  stop(e)
})

tryCatch({
  tar_load(monthly_delta_ndvi.tif)
  message("✓ monthly_delta_ndvi.tif loaded")
}, error = function(e) {
  message("✗ Error loading monthly_delta_ndvi.tif: ", e$message)
  stop(e)
})
...
# Extract NDVI values with simpler approach
protected_areas_monthly_delta_ndvi <- tryCatch({
  # Use exact function to avoid quantile issues
  ext_result <- extract(monthly_delta_ndvi.tif, protected_areas, 
                        fun = function(x) {
                          if(length(x) == 0 || all(is.na(x))) {
                            return(c(NA, NA, NA, NA, NA))
                          }
                          quantile(x, probs = c(0, 0.25, 0.5, 0.75, 1.0), na.rm = TRUE)
                        })
  
  ext_result |>
    as_tibble() |>
    select(-ID) |> #drop the ID column
    setNames(c("Q0", "Q25", "Q50", "Q75", "Q100")) |>
    bind_cols(name = protected_areas$name)
}, error = function(e) {
  message("Error in extract: ", e$message)
...
```{r, echo=F, warning=F}
years_since_fire <- tryCatch({
  ext_result <- extract(years_since_fire.tif, protected_areas, 
                        fun = function(x) {
                          if(length(x) == 0 || all(is.na(x))) {
                            return(c(NA, NA, NA, NA, NA))
                          }
