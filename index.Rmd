...
```{r make_workfow, echo=F, eval = FALSE, message = F, include=FALSE}
...
```{r plot_workflow, echo=F, eval = FALSE, message = F, fig.width = 10, fig.height = 6}
...
```{r p1, echo=F, eval=F, warning=F, message=FALSE}
betas=model_results %>% 
  filter(type=="beta")
...
```{r plot, echo=F, eval=F, message=FALSE,fig.height=12}
...
```{r p2, echo=F, eval=T, warning=F, message=FALSE}
...
```{r plot2, echo = F, eval = TRUE, message = FALSE, fig.height = 12}
