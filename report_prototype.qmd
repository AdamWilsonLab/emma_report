...
```{r debug1, echo = FALSE, warning = FALSE, message = FALSE}
  ...
  focal_weather %>%
    as.data.frame() -> focal_weather #for some reason, this code throws an error without this line.  no idea why.
```  

```{r debug2.0, echo = FALSE, warning = FALSE, message = FALSE}
    
#this bit is causing issues for some reason
    focal_weather %>%
    dplyr::mutate(year = YEAR) -> focal_weather



```

```{r debug2.1, echo = FALSE, warning = FALSE, message = FALSE}
    
#this bit is causing issues for some reason
    focal_weather %>%
    dplyr::mutate(month = MONTH) -> focal_weather

```

```{r debug2.2, echo = FALSE, warning = FALSE, message = FALSE}
    
#this bit is causing issues for some reason
    focal_weather %>%
    dplyr::mutate(day = DAY) -> focal_weather



```

```{r debug2.3, echo = FALSE, warning = FALSE, message = FALSE}
    
#this bit is causing issues for some reason
    focal_weather %>%
    dplyr::mutate(station_name = NAME,
                  mean_precip_mm = PRCP,
                  temp_c = TEMP) -> focal_weather



```

```{r debug3, echo = FALSE, warning = FALSE, message = FALSE}

    focal_weather %>%
    dplyr::mutate(date_time = paste(year,month,day,sep = "-")) -> focal_weather
  
```

```{r debug4, echo = FALSE, warning = FALSE, message = FALSE}

    focal_weather %>%
    dplyr::mutate(date_time = strptime(date_time,format = "%Y-%m-%d")) -> focal_weather

```

```{r debug5, echo = FALSE, warning = FALSE, message = FALSE}
