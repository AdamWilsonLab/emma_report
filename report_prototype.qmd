...
```{r identify_closest_stations, echo = verbose, warning = verbose, message = verbose}
...
```{r download_weather_data, echo = verbose, warning = verbose, message = verbose}
  ...
  #Hack in case some of the files aren't downloaded correctly.
    ...
    if(verbose){message("Removing any non-downloaded stations")}
...
```{r prep_weather_data, echo = verbose, warning = verbose, message = verbose}

    if(verbose){message("Opening weather datasets")}
  ...
  # Filter out oldest data
    ...
    if(verbose){message("Filtering old weather data")}

    focal_weather <-
      focal_weather %>%
      dplyr::filter(YEARMODA > min_date)
    
    
```


```{r prep_precip_data, echo = verbose, warning = verbose, message = verbose}


  #Generate precip dataset

  if(verbose){message("Generating precip dataset")}

focal_weather %>%
  dplyr::select(STNID,NAME,YEAR,MONTH,DAY,PRCP) %>%
  dplyr::group_by(STNID,NAME,YEAR,MONTH) %>%
  dplyr::summarise(n_NAs = sum(is.na(PRCP)),
                   n_days = n(),
                   frac_NAs = n_NAs/n_days) %>%
  full_join(y = focal_weather,multiple="all") %>%
  dplyr::mutate(PRCP = case_when(frac_NAs > 0.5 ~ NA,
                                 frac_NAs <= 0.5 ~ PRCP)) %>% #this line sets months to NA if most of the data are missing
  dplyr::group_by(STNID,NAME,YEAR,MONTH) %>%
  dplyr::summarise(monthly_precip_mm = sum(na.omit(PRCP))) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(date_time = paste(YEAR,MONTH,01,sep = "-")) %>%
  dplyr::mutate(date_time = strptime(date_time,format = "%Y-%m-%d")) %>%
  dplyr::mutate(date_time = lubridate::floor_date(date_time)-1) %>%
  dplyr::filter(!is.na(date_time)) %>%
  dplyr::select(date_time,station_name = NAME, monthly_precip_mm) %>%
  pivot_wider(values_from = monthly_precip_mm,
               names_from = station_name,
               values_fn = mean) %>%
  dplyr::mutate(doy =  lubridate::yday(date_time) ) %>%
  left_join(x=.,
            y  = dplyr::group_by(., doy) %>%
              dplyr::summarize(mean = mean(c_across(!contains(c("doy","date_time"))),
                                           na.rm=TRUE))
               ) %>%
  dplyr::select(-doy) %>%
...
```{r prep_temp_data, echo = verbose, warning = verbose, message = verbose, eval=TRUE}

  # mean daily temp dataset

  focal_weather %>%
  dplyr::group_by(STNID,NAME,YEAR,MONTH,DAY) %>%
  dplyr::summarise(daily_temp_C = mean(na.omit(TEMP))) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(date_time = paste(YEAR,MONTH,DAY,sep = "-")) %>%
  dplyr::mutate(date_time = strptime(date_time,format = "%Y-%m-%d")) %>%
  dplyr::filter(!is.na(date_time)) %>%
  dplyr::select(date_time, station_name = NAME, daily_temp_C) %>%
...
#Generate sf of station points
  ...
  focal_stations_sf <-
  stations_sf %>%
    dplyr::filter(STNID %in% gsub(pattern = ".gz.parquet",
...
```{r prep_noaa_weather_data, echo = FALSE, eval = FALSE, warning = FALSE, message = FALSE}

  #commenting this out for now.  Don't think we need it since switch to gsod, but keeping it just in case
...
```{r ndvi_and_burns, fig.width = 10, fig.height = 7, echo = FALSE, warning = FALSE, message = FALSE, eval=TRUE}
...
```{r plot_precip, echo=F, eval = TRUE, message = F, fig.width = 10, fig.height = 6, eval=TRUE}
#| fig-cap: 
#|   - "Figure 2. Climate Data from NOAA.  Mean data is a long-term average for each day of the year across these weather stations."
...
```{r plot_temp, echo=F, eval = TRUE, message = F, fig.width = 10, fig.height = 6, eval=TRUE}
#| fig-cap: 
#|   - "Figure 2. Climate Data from NOAA.  Mean data is a long-term average for each day of the year across these weather stations."
...
```{r calc_spi, eval = TRUE, fig.width = 10, fig.height = 7, echo = FALSE, warning = FALSE, message = FALSE, eval=TRUE}
...
require(ggplot2)
  ...
  focal_precip %>%
    ...
    mutate(monthly_precip_mm = as.numeric(monthly_precip_mm)) %>%
    group_by(station_name)%>%
    summarize(fraction_na = sum(is.na(monthly_precip_mm))/length(monthly_precip_mm),
              fraction_zero = sum(monthly_precip_mm ==0 |
                                 is.na(monthly_precip_mm))/length(monthly_precip_mm)) %>%
    filter(fraction_na < 0.50) %>%
    filter(fraction_zero < 0.80) %>% #filtering out these low-rainfall ones because they break the SPI
