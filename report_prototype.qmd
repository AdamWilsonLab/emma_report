...
# Process the station and weather data
  ...
  #commenting this out for now.  Don't think we need it since switch to gsod
    # stations %>%
  ...
  # Filter out oldest data
    ...
    focal_weather <- 
      focal_weather%>%  
      filter(YEARMODA > min_date)

  #Generate precip dataset
  
focal_weather %>%
  select(STNID,NAME,YEAR,MONTH,DAY,PRCP) %>%
  group_by(STNID,NAME,YEAR,MONTH) %>%
  summarise(n_NAs = sum(is.na(PRCP)),
            n_days = n(),
            frac_NAs = n_NAs/n_days) %>%
  full_join(y = focal_weather,multiple="all") %>%
  mutate(PRCP = case_when(frac_NAs > 0.5 ~ NA,   
                          frac_NAs <= 0.5 ~ PRCP)) %>% #this line sets months to NA if most of the data are missing
  group_by(STNID,NAME,YEAR,MONTH) %>%
  summarise(monthly_precip_mm = sum(na.omit(PRCP)))%>%
  ungroup() %>%
  ...
  focal_weather%>%
  group_by(STNID,NAME,YEAR,MONTH,DAY)%>%
  summarise(daily_temp_C = mean(na.omit(TEMP)))%>%
  ungroup() %>%
