...
| Data            | Date | Download |
|-----------------|----------------------|-----------------|
| Website updated | `r lubridate::now()|> format("%H:%M on %B %d, %Y")`               | 39.2            |
...
# get sf geometry for park
tar_load(parks)
focal_park=filter(parks$national_parks,CUR_NME==focal_park_name)
...
```{r ndvi_and_burns, fig.width = 10, fig.height = 7, echo = FALSE, warning = FALSE, message = FALSE}
  ...
  addPolygons(data = fires_wgs_truncated,
    ...
    leaflet::addLegend(position = "topright",
                     pal = delta_ndvi_pal,          
                     values = ~values(monthly_delta_ndvi_raster),
                     #values = ~c(-1,1),
                     opacity = 1,
                     title = "delta<br/>NDVI") %>%
