...
```{r precip, eval = FALSE,fig.width = 10, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r weather-dyplot, echo = FALSE, message = FALSE, warning = FALSE}
...
# see: https://rstudio.github.io/dygraphs/
  ...
  # Get data for closest weather stations
    ...
    cent <- st_centroid(focal_park)
    
    #st_crs(focal_park)==st_crs(stations_sf)
    
    
    closest_stations <- 
      data.frame(station = stations_sf$station_name,
    ...
    pb_download(file = closest_stations,
                dest = temp_directory,
  ...
  focal_weather%>%
    dplyr::select(date_time, usaf_station, temp_c) %>%
